...
var mapPinMainWidth = window.data.mapPinMainElem.offsetWidth;
var mapPinMainHeight = window.data.mapPinMainElem.offsetHeight;
var PIN_MAIN_INDENT = 15; //отступ по Y для конца метки
...
var valueOfAddressInput = function () {
  if (window.data.mapElement.classList.contains('map--faded') === true) {
    positionXAddress = Math.floor(window.data.mapPinMainElem.offsetLeft + mapPinMainWidth * 0.5);
    positionYAddress = Math.floor(window.data.mapPinMainElem.offsetTop + mapPinMainHeight * 0.5);
...
window.data.mapPinMainElem.addEventListener('mousedown', function () {
  positionXAddress = Math.floor(window.data.mapPinMainElem.offsetLeft + mapPinMainWidth * 0.5);
  positionYAddress = Math.floor(window.data.mapPinMainElem.offsetTop + mapPinMainHeight + PIN_MAIN_INDENT);
  inputAddress.value = positionXAddress + ', ' + positionYAddress;
})
...
window.data.addFormElement.addEventListener('submit', function (evt) {
  evt.preventDefault()
})
})()
